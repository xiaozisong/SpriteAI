...
const INIT_FORM: ChapterFormModel = {
...
export const ChapterGenerateDialog = ({
  open,
  onOpenChange,
  promptData,
  onOpenMarket,
  ...
  const [formModel, setFormModel] = useState<ChapterFormModel>({ ...INIT_FORM })
  ...
  useEffect(() => {
    if (open) {
      trackEvent('AI Tool', 'Click', 'Chapter')
    }
  }, [open])
  ...
  const isFormModelChanged = useCallback((): boolean => {
    ...
    const theme = formModel.generateType === 'current'
      ? (formModel.extra || '')
      : (formModel.workType || '')
  ...
  const postStreamHandler = useCallback(async () => {
    ...
    try {
      await chapterPostStream(requestData, handleStreamData, handleStreamError, handleStreamComplete, { signal: streamAbortControllerRef.current.signal })
    } catch (err) {
  ...
  const validate = useCallback((): boolean => {
    let valid = true
    setConnectedWorkError(''); setWorkTypeError(''); setCoreMemeError('')
    if (formModel.generateType === 'current') {
      ...
      if (!formModel.coreMeme?.trim()) { setCoreMemeError('请输入核心梗'); valid = false }
    }
    return valid
  }, [formModel])
  ...
  const updateForm = <K extends keyof ChapterFormModel>(key: K, value: ChapterFormModel[K]) =>
    setFormModel(prev => ({ ...prev, [key]: value }))

  const handleSelectRecommend = (key: string, value: string) => {
    if (key in formModel) updateForm(key as keyof ChapterFormModel, value as any)
  }
    ...
    <Dialog open={open} onOpenChange={o => !o && onOpenChange(false)}>
      <DialogContent className="w-[1020px] max-w-[90vw] py-11 px-25 pb-8 sm:max-w-[90vw]" showCloseButton>
        <DialogHeader className="px-5 relative h-9 min-h-0 shrink-0 gap-0">
          ...
          <DialogTitle className="h-9 min-h-0 overflow-hidden text-center text-2xl leading-9">正文生成器</DialogTitle>
        </DialogHeader>
            ...
            <ScrollArea className='h-full w-full'>
              ...
              <div className="min-w-0">
                ...
                <RadioGroup value={formModel.generateType} onValueChange={v => updateForm('generateType', v as GenerateType)} className="mt-1.5 flex flex-row gap-4">
                  <div className="flex items-center gap-2">
                    <RadioGroupItem value="current" id="chapter-gen-type-current" />
                    <label htmlFor="chapter-gen-type-current" className="cursor-pointer text-sm">根据当前作品内容生成</label>
                  </div>
                  <div className="flex items-center gap-2">
                    <RadioGroupItem value="custom" id="chapter-gen-type-custom" />
                    <label htmlFor="chapter-gen-type-custom" className="cursor-pointer text-sm">根据自定义需求生成</label>
                  </div>
                </RadioGroup>
              </div>
              ...
              {formModel.generateType === 'current' ? (
                ...
                <>
                  <div className="min-w-0">
                    <FormRecommendLabel label="文章类型" required recommends={(recommendConfig.workType || []).map(v => ({ label: v, value: v }))} fieldKey="workType" onSelect={handleSelectRecommend} className="mb-1.5 min-w-0" />
